...
export default class Popup {
  ...
  static fillExistingInputs(value) {
    if (typeof value === "object") {
      const priorityText = popupForm.querySelector(
  ...
  static activateSelection(select) {
    const container = document.querySelector(
      '[data-select="' + select.getAttribute("data-select") + '"]'
    );
      ...
      options.forEach((option) =>
        option.addEventListener("click", () => {
          ...
          if (select.hasAttribute("data-priority")) {
            select.setAttribute(
              "data-priority",
              option.getAttribute("data-priority")
            );
          }
          container.classList.toggle("active");
        })
  ...
  static addProject(ev, value) {
    ...
    newProjectOption.addEventListener(
      "click",
      this.chooseOption(popupProjectSelect, popupProjectSelection, true)
    );
  }
  ...
  static createTask(ev) {
    ...
    if (popupInputs.isEdit) this.editTask(popupInputs.task);
      ...
      popupInputs.project.innerHTML =
        '<img src="./images/inbox.svg" alt="" class="icon">Inbox</div>';
      popupInputs.priority.innerHTML =
        '<img src="./images/priority.svg" alt="">Priority 4';
  ...
  static editTask(task) {
    ...
    inboxObj.tasks[task.globalIndex].priority =
      popupInputs.priority.getAttribute("data-priority");
